...
?>
	...
	<tr>
		...
		<td>
			<div class="input-group datetime">
				<input type="text" name="search_start_day" class="form-control input-sm text-center" value="<?=$search_start_day?>"/>
			...
			~
			<div class="input-group datetime">
				<input type="text" name="search_end_day" class="form-control input-sm text-center" value="<?if(empty($search_end_day)){echo date("Y-m-d");}else{echo $search_end_day;}?>"/>
				<span class="input-group-addon">
					<span class="glyphicon glyphicon-calendar"></span>
				</span>
			</div>
	...
	$trade_stat = 5;
	if($search_start_day){
		$result=$db->result("select sum(t.trade_total_price), t.userid, t.name, count(*), m.connect, sum(t.trade_use_point), sum(t.trade_save_point), t.sale_end_day from cs_trade t, cs_member m where t.userid is not null and t.userid=m.userid and t.trade_stat='$trade_stat' and (DATE_FORMAT(t.trade_day,'%Y-%m-%d')>='$search_start_day' and DATE_FORMAT(t.trade_day,'%Y-%m-%d')<='$search_end_day') group by userid order by 1 desc");
	}else{
		$result=$db->result("select sum(t.trade_total_price), t.userid, t.name, count(*), m.connect, sum(t.trade_use_point), sum(t.trade_save_point) from cs_trade t, cs_member m where t.userid is not null and t.userid=m.userid and t.trade_stat='$trade_stat' group by userid order by 1 desc");
	}
