...
for($i=0;$i<count($arrArea);$i++) {
	...
	{
		$result=$db->result("select trade_total_price, trade_day, order_gu from cs_trade where DATE_FORMAT(trade_day,'%Y-%m-%d')>='$search_sday' and DATE_FORMAT(trade_day,'%Y-%m-%d')<='$search_eday' and trade_stat='$trade_stat' and order_gu='y' and add1 like '$arrArea[$i]%'");
	}
	else if($search_sday && empty($trade_stat))
	{
		$result=$db->result("select trade_total_price, trade_day, order_gu from cs_trade where DATE_FORMAT(trade_day,'%Y-%m-%d')>='$search_sday' and DATE_FORMAT(trade_day,'%Y-%m-%d')<='$search_eday' and order_gu='y' and add1 like '$arrArea[$i]%'");
	}
	else if(empty($search_sday) && $trade_stat)
	{		
		$result=$db->result("select trade_total_price, trade_day, order_gu from cs_trade where trade_stat='$trade_stat' and order_gu='y' and add1 like '$arrArea[$i]%'");
	}
	else
	{
		$result=$db->result("select trade_total_price, trade_day, order_gu from cs_trade where order_gu='y' and add1 like '$arrArea[$i]%'");		
	}
