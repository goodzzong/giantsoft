...
include $_SERVER['DOCUMENT_ROOT']."/common.php";
 ...
 if($_POST[content]) {
	...
	$content=$tools->strHtml($_POST[content]); 
	 ...
	 // 신규회원가입
	 $content = str_replace("[{USER_NAME}]","회원 이름", $content);
	 $content = str_replace("[{USER_ID}]","회원 아이디", $content);
	 $content = str_replace("[{USER_PASSWD}]","회원 패스워드", $content);
	 $content = str_replace("[{USER_EMAIL}]","회원 이메인", $content);
	 $content = str_replace("[{USER_TEL}]","회원 전화번호", $content);
	 $content = str_replace("[{USER_PHONE}]","회원 휴대번호", $content);
	 $content = str_replace("[{USER_ADDRESS}]","회원 주소", $content);
