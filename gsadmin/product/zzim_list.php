...
function send(form){
	...
	if( count == 0 ) {
		...
		form.action = "./zzim_exe.php?code=<? echo $code ?>";
		form.submit();
	}
...
<div class="col-md-12" >
	...
	if($part_idx) {
		...
		while( $part_stat_row = @mysql_fetch_object( $part_result )) {
			$i++;
			$part_name.="카테고리 : <font color='#FF0000'>".$part_stat_row->part_name."</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
		}
	} else {
		$part_name = "전체";
	}
	...
	<tr>
		...
		<td class="text-center"><img src="<?=$site_url?>/data/goodsImages/<?=$row->images1?>" class="img-rounded" style="width:50px;height:50px;"></td>
