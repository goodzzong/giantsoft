	...
	$query		= "select * from $table where order_gu='y'";
		...
		if($search_order){
			if($search_item){
				...
				$query.=" and (trade_code like '%$search_order%' or name like'%$search_order%' or userid like'%$search_order%' or phone like'%$search_order%')";
			}
		}
...
?>
			...
			<div class="form-group">
				<div class="input-group-btn">
					<select name="search_item" class="form-control input-sm" >
						...
						<option value="userid" <?if($search_item=="userid"){?>selected<?}?>>회원ID</option>
						<option value="phone" <?if($search_item=="phone"){?>selected<?}?>>연락처</option>
					</select>
				</div>
			</div>
	...
	<tr>
		...
		<td class="text-center"><?=$row[name];?></a><?if($row[device]=="mobile"){?>&nbsp;<span class="badge">M</span><?}?></td>
