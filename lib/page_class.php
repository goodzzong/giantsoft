...
class Page {
	...
	function bbs_page( $code, $table, $totalPage, $totalList, $listScale, $pageScale, $startPage, $prexImgName, $nextImgName, $search_item=0, $search_order="") {
		if( $totalList > $listScale) {
			if($totalList> 10){
				$mv_data=$this->encode("startPage=0&code=".$code."&table=".$table."&search_item=".$search_item."&search_order=".$search_order);
				echo "<a href='$_SERVER[PHP_SELF]?bbs_data=$mv_data' class='first'> <img src='/images/button/btn_first.jpg' alt='First' /> </a>";
			}
		}
		...
		if( $totalList > $listScale ) {
			if( $startPage+1 > $listScale*$pageScale ) {
				$prePage = $startPage - $listScale * $pageScale;
				$mv_data=$this->encode("startPage=".$prePage."&code=".$code."&table=".$table."&search_item=".$search_item."&search_order=".$search_order);
				echo "<a href='$_SERVER[PHP_SELF]?bbs_data=$mv_data' class='prev'> <img src='/images/button/btn_prev.jpg' alt='Previous' /> </a>";
			}
			for( $j=0; $j<$pageScale; $j++ ) {
				$nextPage = ($totalPage * $pageScale + $j) * $listScale;
				$pageNum = $totalPage * $pageScale + $j+1;
				if( $nextPage < $totalList ) {
					if( $nextPage!= $startPage ) {
						$mv_data=$this->encode("startPage=".$nextPage."&code=".$code."&table=".$table."&search_item=".$search_item."&search_order=".$search_order);
						echo "<a href='$_SERVER[PHP_SELF]?bbs_data=$mv_data' > $pageNum </a>";
					} else {
			...
			if( $totalList > (($totalPage+1) * $listScale * $pageScale)) {
				$nNextPage = ($totalPage+1) * $listScale * $pageScale;
				$mv_data=$this->encode("startPage=".$nNextPage."&code=".$code."&table=".$table."&search_item=".$search_item."&search_order=".$search_order);
				echo "<a href='$_SERVER[PHP_SELF]?bbs_data=$mv_data' class='next'> <img src='/images/button/btn_next.jpg' alt='Next' /> </a>";
			}
		}
		...
		if( $totalList > $listScale) {
			...
			if($totalList> 10){
				$totalp = ($listScale * (ceil($totalList / $listScale))) - $listScale;
				$mv_data=$this->encode("startPage=".$totalp."&code=".$code."&table=".$table."&search_item=".$search_item."&search_order=".$search_order);
				echo "<a href='$_SERVER[PHP_SELF]?bbs_data=$mv_data' class='last'> <img src='/images/button/btn_last.jpg' alt='Last' /> </a>";
			}
